...
// Connect is the shim interface that wraps libvirt connectivity. This allows
// us to create a mock implementation for testing, as we cannot assume we will
// always have expensive bare-metal hosts to run CI, especially on a public
// repository. Functions should be added as required and match only those
// provided by libvirt.Connect.
//
...
type Connect interface {
	...
	LookupNetworkByName(name string) (Network, error)
}

// Network is the shim interface that wraps libvirt connectivity specific to a
// named network. This allows us to create a mock implementation for testing,
// as we cannot assume we will always have expensive bare-metal hosts to run
// CI, especially on a public repository. Functions should be added as required
// and match only those provided by libvirt.Network.
