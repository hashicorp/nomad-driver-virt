...
var (
	// baseTypes are the chain types that may require a hook.
	...
	// baseHooks are the hooks available to base types.
...
RULES_LOOP:
	...
	// If there are no positioned rules, then we're done here.
	...
	for _, rule := range positionedRules {
		...
		// Adjust the position to be zero indexed, and not less than zero.
		desiredIndex := max(0, int(rule.Position-1))

		// If the desired index already contains an entry, use that as a reference
		// to insert the new rule. Otherwise, add the rule to the end of the list.
		if desiredIndex < len(existingRules) {
			// Grab the reference rule.
			referenceRule := existingRules[desiredIndex]
			...
			// Unset the position value of the rule.
			rule.Position = 0
