...
func Test_nft_Add(t *testing.T) {
	...
	t.Run("ok - add rules - invalid positions", func(t *testing.T) {
		nft := testNft(t)

		// Create a new request and add a chain with a couple rules.
		req := shared.NewRequest(testName("nomad-identifier"))
		chain := shared.Chain{
			Table: "filter",
			Chain: testName("test-chain"),
		...
		// Create a couple new rules, one with a position of a negative value
		// and the other with a position larger than the existing number of
		// rules.
		newRules := []shared.Rule{
			{
				Chain:    chain,
				Position: -2,
				Spec: shared.Spec{
		...
		expectedRules := make([]*nftables.Rule, 0, len(req.Rules())+len(computedRules))
		expectedRules = append(expectedRules, computedUpdatedRules[0]) // First new rule should be first rule.
		expectedRules = append(expectedRules, rules...)                // Existing records should be second and third rule.
		// expectedRules = append(expectedRules, rules[1])
		// expectedRules = append(expectedRules, rules[0])
		expectedRules = append(expectedRules, computedUpdatedRules[1]) // Last new rule should be last rule.
		...
		// The number of rules should be the same as the expected.
		must.Len(t, len(expectedRules), rules)

		// Check that the rules are in the correct positions.
		for i := range len(expectedRules) {
			must.EqFunc(t, expectedRules[i], rules[i], equalRules,
				must.Sprintf("invalid rule at index %d", i))
		}
	})
